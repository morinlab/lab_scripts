...
bedresult_cols = c("chr_var","start_var","end_var","chr_cnv","start_cnv","end_cnv","overlap","TITAN_call","local_cnv","prevalence","purity","patient","purity_corrected","vaf_mean")
...
for (i in 1:dim(snp_whole)[1]){
  ...
  if(dim(bedresult[bedresult['patient']==patient_temp,])[1] > 0){
    ...
    preval=1
    snp_whole[i,"prev1_CCF.estimate"] = ifelse(is.na(preval),NA,vaf.to.ccf(allele_frac,purity,copy_num,preval))
    ...
    preval = ifelse(is.na(snp_whole[i,"Prevalence.estimate"]) & (snp_whole[i,"TITAN_call"]=="HET"),1,snp_whole[i,"Prevalence.estimate"])
    snp_whole[i,"Prevalence.estimate"] = preval
    snp_whole[i,"pur.correc.CCF.estimate"] = ifelse(is.na(preval),NA,vaf.to.ccf(allele_frac,purity,copy_num,preval))
    preval=1
    snp_whole[i,"pur.correc.prev1.CCF.estimate"] = ifelse(is.na(preval),NA,vaf.to.ccf(allele_frac,purity,copy_num,preval))
    ...
    snp_whole[i,"purity.recalculated"] = purity
    copy_num = snp_whole[i,"Local.copy.number"]
    preval = ifelse(is.na(snp_whole[i,"Prevalence.estimate"]) & (snp_whole[i,"TITAN_call"]=="HET"),1,snp_whole[i,"Prevalence.estimate"])
    snp_whole[i,"Prevalence.estimate"] = preval
    snp_whole[i,"vaf.correc.CCF.estimate"] = ifelse(is.na(preval) | is.na(allele_frac),NA,vaf.to.ccf(allele_frac,purity,copy_num,preval))
    preval=1
    snp_whole[i,"vaf.correc.prev1.CCF.estimate"] = ifelse(is.na(preval) | is.na(allele_frac),NA,vaf.to.ccf(allele_frac,purity,copy_num,preval))
  }
}
