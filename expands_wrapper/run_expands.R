...
if(include_loh > 0){
	...
	vaf_loh_event= 1- sequenza_loh[,"Bf"]  #the Expands method expects VAFs that were increased relative to the normal but Sequenza reports BAF relative to lower allele
...
if(include_loh > 0){
	...
	samp_param = paste(sample,loh_string,"SQ_state_INDEL_AQ_", max_score, ",precision_", precision,sep="")
} else{
	merge_snv = snv_data
	samp_param = paste(sample,"SQ_state_INDEL_noLOH_AQ", max_score, "_precision_", precision,sep="")
