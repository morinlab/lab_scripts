...
def main():
	...
	#mpileup object (1)
	my_command = []
	out_normal_mp = "{0}/mpileups/{1}/{2}.mpileup.gz".format(os.path.abspath(os.path.dirname(odir)),patient,ntpath.basename(input_normal).replace(".bam",""))
	out_tumour_mp = "{0}/mpileups/{1}/{2}.mpileup.gz".format(os.path.abspath(os.path.dirname(odir)),patient,ntpath.basename(input_tumour).replace(".bam",""))
	...
	if((doesnt_exist_output(out_tumour_mp))):
    	...
    	# seqz object (2)
	out_patient_seq ="{0}/seqz/{1}/{2}_tumour_seqz.gz".format(os.path.dirname(odir),patient,ntpath.basename(input_normal).replace(".bam",""))
	out_patient_seq_bin ="{0}/seqz/{1}/{2}_tumour_binned.seqz.gz".format(os.path.dirname(odir),patient,ntpath.basename(input_normal).replace(".bam",""))
	...
	#print my_command
        ...
        log_stderr = open(odir + "log_stderr.log",'w+'); log_stderr.write(str(err))
        log_stdout = open(odir + "log_stdout.log",'w+'); log_stdout.write(str(out))
